...
def run_n_simulations(s, e, m, w, js, d, uncertainty_parameters, n_simulations, uncertainty_source: str = 'worker', processing_times: bool = False, machine_breakdowns: bool = False, worker_unavailabilites: bool = False):
    ...
    for i in range(n_simulations):
        g = Graph(s, e, m, w, js)
        g.simulate(d, uncertainty_parameters, processing_times=processing_times, machine_breakdowns=machine_breakdowns,
                   worker_unavailabilities=worker_unavailabilites, uncertainty_source=uncertainty_source)
...
class Graph:
    ...
    def __init__(self, s, e, a, w, js, leftshift: bool = False, buffers: list[float] = []):
        ...
        for i in range(len(s)):
            nodes.append(Node(self.s, self.e, self.m,
                         self.w, self.js, self.b, i))
    ...
    def get_vectors(self):
        ...
        m = []
        w = []
        b = [self.all_nodes[i].buffer if len(
            self.all_nodes[i].children) > 0 else 0.0 for i in range(len(self.all_nodes))]
    ...
    def simulate_processing_times(self, d, wv, uncertainty_source: str = 'worker'):
        ...
        while len(open_list) > 0:
            ...
            for child in current.children:
                self.add_child(child, open_list, closed_list)
            new_duration = self.real_duration(d[current.operation][current.machine][current.worker], wv[current.worker % len(
                wv)]) if uncertainty_source == 'worker' else self.real_duration(d[current.operation][current.machine][current.worker], wv[current.machine % len(wv)])
    ...
    def simulate_machine_breakdowns(self, d):
        ...
        for i in range(n_machines):
            ...
            alpha = random.uniform(makespan_original *
                                   0.1, makespan_original * 0.2)
            up.append([machine_lambda, alpha, 3.602])
        all_events = self.generate_all_events(up)
        operations = []
        for i in range(len(all_events)):
            operation = self.find_affected_operation(
                all_events[i][1][0], all_events[i][1][1], machine=all_events[i][0])
            if operation != -1:
    ...
    def simulate_worker_unavailabilities(self, d):
        ...
        for i in range(n_workers):
            ...
            alpha = random.uniform(makespan_original *
                                   0.5, makespan_original * 1.0)
            up.append([worker_lambda, alpha, 3.602])
        all_events = self.generate_all_events(up)
        operations = []
        for i in range(len(all_events)):
            operation = self.find_affected_operation(
                all_events[i][1][0], all_events[i][1][1], worker=all_events[i][0])
            if operation != -1:
                operations.append((operation, all_events[i][1]))
        if len(operations) == 0:
            return 0
    ...
    def simulate(self, d, wv=None, processing_times: bool = False, machine_breakdowns: bool = False, worker_unavailabilities: bool = False, uncertainty_source: str = 'worker'):
        n_conflicts = 0
        if processing_times:
            n_conflicts += self.simulate_processing_times(
                d, wv, uncertainty_source)
    ...
    def plot_data(self, strict: bool = False):
        ...
        is_critnode = [
            True if node in crit_nodes else False for node in self.all_nodes]
        for i in range(len(self.all_nodes)):
            s.append(self.all_nodes[i].start)
            e.append(self.all_nodes[i].end)
            m.append(self.all_nodes[i].machine)
...
class Node:
    ...
    def update_values(self):
        ...
        change = 0
        parent_end_times = [
            parent.end + ((parent.end-parent.start)*parent.buffer) for parent in self.parents]
        parent_end_times.append(s)
        # if len(parent_end_times) > 0 else 0
        earliest_start = max(parent_end_times)
