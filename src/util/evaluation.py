...
def workload_balance(machine_assignments: list[int], worker_assignments: list[int], durations: list[list[list[int]]]) -> float:
    ...
    for i in range(len(worker_assignments)):
        working_time[worker_assignments[i]
                     ] += durations[i][machine_assignments[i]][worker_assignments[i]]
...
def translate_fjssp(sequence: list[int], machines: list[int], durations: list[list[list[int]]]) -> tuple[list[int], list[int]]:
    ...
    for i in range(len(sequence)):
        ...
        offset = 0
        if operation > 0:
            offset = max(0, end_times[start_index-1] -
                         end_on_workstations[machine])
...
def translate(sequence: list[int], machines: list[int], workers: list[int], durations: list[list[list[int]]], verbose: bool = False) -> tuple[list[int], list[int], list[int]]:
    class TimeSlot:
    ...
    def get_start_index(job: int, job_sequence: list[int]) -> Optional[int]:
        for i in range(len(job_sequence)):
            if job_sequence[i] == job:
                return i
        return None
    ...
    start_indices = [get_start_index(job, job_sequence)
                     for job in sorted(list(jobs))]
    n_operations = len(durations)
    n_machines = len(durations[0])
    n_workers = len(durations[0][0])
    next_operation: list[int] = [0] * len(jobs)
    end_on_machines: list[list[TimeSlot]] = [
        [TimeSlot(0, 0)] for _ in range(n_machines)]
    end_of_workers: list[list[TimeSlot]] = [
        [TimeSlot(0, 0)] for _ in range(n_workers)]
    ...
    for i in range(n_operations):
        ...
        if verbose:
            print(
                f"job={job}, start_idx={start_index}, machine={machine}, worker={worker}  ->  duration={duration}")
        if duration == 0:
        ...
        if len(end_of_workers[worker]) > 0:
            worker_earliest: TimeSlot = earliest_fit(
                end_of_workers[worker], TimeSlot(offset, offset + duration))
